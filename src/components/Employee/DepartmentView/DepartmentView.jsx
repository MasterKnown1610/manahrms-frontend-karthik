...
import { MdPeople, MdGroups, MdBusiness, MdTrendingUp } from 'react-icons/md';
...
const DepartmentView = () => {
  ...
  const departmentsList = Object.values(departments);
  const filteredDepartments = selectedDept === 'all' 
    ? departmentsList 
    : departmentsList.filter((dept) => dept.name === selectedDept);
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Card>
              <Statistic
                title="Avg Team Size"
                value={(employeesData.length / departmentsList.length).toFixed(1)}
